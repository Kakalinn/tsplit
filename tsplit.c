...
int main(int argc, char** argv)
{
	...
	assert(w);
	char fn[1024] = "tsplit-file-%d.txt";
	char c, ff[1024 + 100]; // Only a problem if you splitting file into more than a 10^100 pieces.
	for (i = 0; i < 256; i++) s[i] = 0;
	init_seperator_mask(s, " \n");
	inppar_fetchi(argc, argv, "-m", &m);
	inppar_fetchi(argc, argv, "-w", &maxw);
	inppar_fetchi(argc, argv, "--word-size", &maxw);
	inppar_fetchs(argc, argv, "-n", fn);
	inppar_fetchs(argc, argv, "--name", fn);
